...
export default function Stripe() {
  ...
  useEffect(() => {
    const fetchCards = async () => {
      try {
        const res = await getStripeCards();
  ...
  const handleViewDetails = (ulid) => {
    router.push(
      `/dashboard/Virtual-Card-Stripe/StripeCardDetails?card_id=${ulid}`,
    );
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8">
        ...
        {card_create_action && (
          <Link
            href="/dashboard/Virtual-Card-Stripe/CreateVirtualCard"
      ...
      {myCards.length === 0 ? (
        <div className="bg-gray-50 border-2 border-solid border-gray-300 rounded-2xl p-12 text-center mt-5">
          ...
          {card_create_action && (
            <Link href="/dashboard/Virtual-Card-Stripe/CreateVirtualCard">
      ...
      <div className="bg-white rounded-2xl shadow border border-gray-200 overflow-hidden mt-12">
        <div className="px-6 py-5 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-500 mt-1">
            All activity related to your virtual cards (Stripe)
          </p>
        </div>
