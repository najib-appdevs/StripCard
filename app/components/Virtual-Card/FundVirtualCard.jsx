...
import FundVirtualCardSkeleton from "./FundVirtualCardSkeleton";

export default function FundVirtualCard() {
  // ============================================================================
  // ROUTER & PARAMS
  // ============================================================================
  ...
  const [serverResponseMsg, setServerResponseMsg] = useState(null);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const [chargesRes, cardRes, dashRes] = await Promise.all([
          getStrowalletCharges(),
          getStrowalletCardDetails(cardId),
          getUserDashboard(),
        ]);
        ...
        setCardData(cardRes?.data?.myCards || null);
  ...
  const charge = useMemo(() => chargesData?.cardCharge || {}, [chargesData]);
  ...
  const fixedCharge = Number(charge.fixed_charge) || 2;
  const percentCharge = Number(charge.percent_charge) || 1;
  ...
  // TRANSACTION LIMITS
  // ============================================================================
  const transactionMin = Number(charge.min_limit) || 1;
  const transactionMax = Number(charge.max_limit) || 1000;
  const dailyLimit = Number(charge.daily_limit) || 10000;
  const monthlyLimit = Number(charge.monthly_limit) || 50000;
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      card_id: cardId,
      fund_amount: numericAmount,
      currency: currency,
      from_currency: fromWallet,
    };

    try {
      const response = await fundStrowalletCard(payload);
        ...
        <div className="overflow-hidden rounded-2xl bg-white shadow-xl">
          <div className="p-6 sm:p-8 lg:p-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-7">
              ...
              {serverResponseMsg && (
                <div
                  className={`rounded-lg p-4 text-center text-sm font-medium border ${
                    serverResponseMsg.toLowerCase().includes("success")
